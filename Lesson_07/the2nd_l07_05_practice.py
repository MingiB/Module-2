# the2nd_l07_05_practice.py
"""
Дан список словарей фильмов  movies. Каждый словарь в списке содержит два ключа: «name» и «movies».
Используя dict comprehension необходимо:
1) Получить словарь movies_by_actor с одной парой ключ-значение для каждого актера в формате: 
{‘Angelina Jolie': [<список фильмов>], 'Brad Pitt': [<список фильмов>], 'George Clooney': [<список фильмов>]}
2) Используя полученный словарь movies_by_actor и dict comprehension получить новый словарь m_movies, который будет содержать только фильмы, начинающиеся на букву «М» для каждого актера
...
movies_by_actor = {i['name']:i['movies'] for i in movies}
m_movies = {i:[k for k in j if k[0]=='М']for i,j in movies_by_actor.items()}


print("Задание 1:", movies_by_actor)
print("Задание 2:", m_movies)
